...
int mrutils::mysql::mysql_sha1_reset(SHA1_CONTEXT *context)
{
#ifndef DBUG_OFF
  if (!context)
    return SHA_NULL;
...
int mrutils::mysql::mysql_sha1_result(SHA1_CONTEXT *context,
...
int mrutils::mysql::mysql_sha1_input(SHA1_CONTEXT *context, const uint8_t *message_array,
                     unsigned length)
{
  if (!length)
    return SHA_SUCCESS;
